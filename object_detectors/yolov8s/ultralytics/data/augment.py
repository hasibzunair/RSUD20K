...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        """Applies MixUp augmentation as per
...
class RandomHSV:
    ...
    The adjustments are random but within limits set by hgain, sgain, and
    vgain.
    """
    ...
    def __call__(self, labels):
        ...
        The modified image replaces the original image in the input 'labels'
        dict.
        """
        img = labels["img"]
...
class CopyPaste:
    """Implements the Copy-Paste augmentation as described in the paper
    https://arxiv.org/abs/2012.07177.

    This class is responsible for applying the Copy-Paste augmentation on
    images and their corresponding instances.
    """
...
class Albumentations:
    ...
    Optional, uninstall package to disable. Applies Blur, Median Blur, convert
    to grayscale, Contrast Limited Adaptive Histogram Equalization, random
    change of brightness and contrast, RandomGamma and lowering of image
...
def classify_transforms(
    size=224, rect=False, mean=(0.0, 0.0, 0.0), std=(1.0, 1.0, 1.0)
    ...
    transforms = [
        ClassifyLetterBox(size, auto=True) if rect else CenterCrop(size),
        ToTensor(),
    ]
