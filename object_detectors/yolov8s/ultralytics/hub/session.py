...
class HUBTrainingSession:
    ...
    def _handle_signal(self, signum, frame):
        ...
        This method does not use frame, it is included as it is passed by
        signal.
        """
    ...
    def upload_metrics(self):
        ...
        smart_request(
            "post", self.api_url, json=payload, headers=self.auth_header, code=2
        )
    ...
    def _get_model(self):
        ...
        try:
            response = smart_request(
                "get", api_url, headers=self.auth_header, thread=False, code=0
            )
            ...
            if data["status"] == "new":  # new model to start training
                self.train_args = {
                    "batch": data[
                        "batch_size"
                    ],  # note HUB argument is slightly different
                    "epochs": data["epochs"],
                    "imgsz": data["imgsz"],
                ...
                self.model_file = data.get("cfg") or data.get(
                    "weights"
                )  # cfg for pretrained=False
