...
def Args():
    ...
    parser.add_argument(
        ...
        default=640,
        type=int,
        help="Image size (pixels) for training and validation.",
    )
    parser.add_argument(
        "--batch_size", default=12, type=int, help="Total batch size for training."
    )
    parser.add_argument(
        "--epochs",
        default=400,
    ...
    parser.add_argument(
        ...
        default=0,
        type=str,
        help='CUDA device(s) i.e 0,1,2 to use or "cpu" for CPU training.',
    )
...
def main():
    ...
    if not os.path.isfile(args.yaml_path):
        raise FileNotFoundError(
            f"The specified YAML file was not found: {args.yaml_path}"
        )
